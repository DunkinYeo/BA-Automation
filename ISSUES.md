...
| CODE-11 | ✅ | Phase 2 setup | Phase 2 연결 시 302 팝업 → INFO 화면 미진입 → Phase 2 TC 전체 실패 | Phase 1 에러 TC에서 남은 BLE 세션으로 서버가 302 반환, 재연결 로직 없음 | `main.py` Phase 2 연결에 에러 팝업 dismiss + 5s 대기 + 최대 3회 재시도 추가 |
...
| APP-07 | ⚠️ | Phase 2 연속 테스트 | BLE 세션 남은 상태에서 재연결 시 "알 수 없는 오류 (302)" 팝업 — 팝업 닫으면 정상 진행 가능 |
| APP-06 | ⚠️ | LOGIN-ERR-11 | GPS OFF 상태에서도 BLE 연결 성공 후 앱 진행 가능 — 로그인 단계에서 GPS 에러 팝업 미표시 (TC 제거, 앱 이슈로 개발팀 전달) |
...
| 2026-05-14 | CODE-11 해결 — Phase 2 연결 302 팝업 처리, 재시도 로직 추가 |
| 2026-05-14 | TC 범위 조정 — GPS OFF TC 전체 제거 (login-err-11, exam-err-02); BT/WiFi OFF TC는 로그인 단계 제거, 검사 중(connectivity) 유지 |
| 2026-05-14 | APP-05 제거 — 버그 아님 (React Native 버튼 시각적 비활성화는 접근성 속성 미반영이 의도된 동작) |
